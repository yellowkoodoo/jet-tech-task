...
test.describe('API | ORDERS', () => {
    ...
    test('Create order', async ({ request }) => {
        ...
        expect(orderById.id).toEqual(order1.items[0].productId);
    ...
    test('Empty items', async ({ request }) => {
        ...
        expect(created).toEqual('items array is required and must not be empty');        
    })
    ...
    test('Invalid product id', async ({ request }) => {
        ...
        const created = await orderCreate(request, order1, false);
        console.log(created);
        expect(created).toEqual(`Product ${order1.items[0].productId} not found`);      
    })
})
